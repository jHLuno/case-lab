...
with previous_settings as (
  ...
  where environment = 'live'
    and early_bird_amount_minor is distinct from 1500000
  for update
), updated_settings as (
  update public.case_lab_3_event_settings settings
  set early_bird_amount_minor = 1500000,
...
select
  updated.environment,
  'event_settings_live_early_bird_price_updated',
  ...
  jsonb_build_object(
    'earlyBirdAmountMinor', 1500000,
    'configurationVersion', updated.configuration_version
  ),
  'migration',
  '20260922010000_update_case_lab_3_live_early_bird_price'
