-- Keep participant answers concise for live case responses.
alter table public.case_lab_3_live_submissions
  drop constraint if exists case_lab_3_live_submissions_answer_text_check;

alter table public.case_lab_3_live_submissions
  add constraint case_lab_3_live_submissions_answer_text_check
  check (char_length(btrim(answer_text)) between 1 and 200);
...
begin
  ...
  if p_environment not in ('test', 'live')
     ...
     or char_length(v_answer) > 200 then
