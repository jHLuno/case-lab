# Agent setup for `icomat-replica`

This is a deepsec scanning workspace. Project `icomat-replica` was just registered
(target: ``). Setup is incomplete — `data/icomat-replica/INFO.md`
...
2. **Fill in `data/icomat-replica/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id icomat-replica
pnpm deepsec process --project-id icomat-replica
```
