...
export async function handlePatch(request: Request, context: { params: Promise<{ id: string }> }, dependencies: Partial<Dependencies> = {}): Promise<Response> {
  ...
  try {
    ...
    if (!isRecord(body) || !Array.isArray(body.entries) || body.entries.length > 100) return noStoreJson({ error: "invalid_request" }, { status: 400 });
    ...
    await active.updateShortlist({ caseId: id, entries: entries as Array<{ submissionId: string; included: boolean; finalOrder: number | null; operatorReason: string | null }> });
