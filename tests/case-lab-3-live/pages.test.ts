...
test("participant styles preserve keyboard focus and reduced-motion behavior", async () => {
  ...
  assert.match(source, /\.question\s*\{[\s\S]*?overflow-wrap:\s*normal/u);
  assert.match(source, /\.question\s*\{[\s\S]*?word-break:\s*normal/u);
  assert.match(source, /@media\s*\(max-width:\s*520px\)[\s\S]*?\.question\s*\{[\s\S]*?font-size:\s*21px/u);
});
